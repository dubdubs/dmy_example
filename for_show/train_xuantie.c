...
#define BANNER_ROWS 10  /* 顶框+空行+5行大字+空行+副标题+底框 */

/* -----------------------------------------------------------------------
 *  "XUANTIE" 5行高轮廓字体（已在 banner_demo 验证）
 *  每字7列，字间距2空格，总显示宽 = 7*7 + 6*2 = 61列
 * ---------------------------------------------------------------------- */
#define BIG_NLETTERS  7
#define BIG_FONT_ROWS 5
#define BIG_FONT_COLS 7
#define BIG_LETTER_GAP 2
#define BIG_TOTAL_DW (BIG_NLETTERS * BIG_FONT_COLS + (BIG_NLETTERS-1) * BIG_LETTER_GAP)

static const char *BIG_FONT[BIG_NLETTERS][BIG_FONT_ROWS] = {
    /* X */ { "##   ##", " ## ## ", "  ###  ", " ## ## ", "##   ##" },
    /* U */ { "##   ##", "##   ##", "##   ##", "##   ##", " ##### " },
    /* A */ { "  ###  ", " ## ## ", "#######", "##   ##", "##   ##" },
    /* N */ { "##   ##", "###  ##", "## # ##", "##  ###", "##   ##" },
...
/* 彩虹色表 */
static const char *BIG_RAINBOW[] = {
    "\033[1;91m", "\033[1;93m", "\033[1;92m", "\033[1;96m",
    "\033[1;94m", "\033[1;95m", "\033[1;97m",
};
static const char *BIG_DIM[] = {
    "\033[31m", "\033[33m", "\033[32m", "\033[36m",
    "\033[34m", "\033[35m", "\033[37m",
};
#define BIG_NCOLORS 7
#define BIG_RESET "\033[0m"
...
int my_mvaddstr(int y, int x, const char *str) {
    int real_y = y + BANNER_ROWS;   /* 偏移到横幅下方 */
    if (real_y < BANNER_ROWS || real_y >= LINES + BANNER_ROWS) return OK;
    ...
    printf("\033[%d;%dH", real_y + 1, start_x + 1);
...
/* ---------------------------------------------------------------
 *  draw_banner: 每帧在屏幕顶部绘制庆祝横幅（共 3 行，行 0-2）
 *  横幅宽度自适应 COLS，内容居中对齐
 * --------------------------------------------------------------- */
    ...
    // while (running) {
        ...
        for (x = COLS - 1; running; --x) {
            ...
            /* ---- 顶部彩虹横幅（每帧更新，banner_frame驱动动画）----
             *  行1: 波浪边框
             *  行2: 空行
             *  行3-7: 5行彩虹大字 XUANTIE
             *  行8: 空行
             *  行9: CONGRATULATIONS（变色）
             *  行10: 波浪边框（= BANNER_ROWS+1，但只画到 BANNER_ROWS）
             * -------------------------------------------------------- */
            {
...
#define BPUTCH(c)  do { buf[pos++]=(c); } while(0)
#define BPUTESC(s) do { int _l=strlen(s); memcpy(buf+pos,s,_l); pos+=_l; } while(0)
                ...
                for (i = 1; i < max_col - 1; i++)
                    BPUTCH(wave[(i + banner_frame) % 4]);
                BPUTCH('+');
                BPUTESC(BIG_RESET);
                buf[pos] = '\0';
                printf("\033[1;1H\033[2K"); fwrite(buf, 1, pos, stdout);

                /* 行2：空行 */
                pos = 0; BPUTCH('|');
                for (i = 0; i < inner; i++) BPUTCH(' ');
                BPUTCH('|'); buf[pos] = '\0';
                printf("\033[2;1H\033[2K"); fwrite(buf, 1, pos, stdout);

                /* 行3-7：彩虹大字 */
                {
                    int breath  = (banner_frame / 16) % 2;
                    int padl    = (inner - BIG_TOTAL_DW) / 2;
                    int padr    = inner - BIG_TOTAL_DW - padl;
                    int row, li, ci;
                    for (row = 0; row < BIG_FONT_ROWS; row++) {
                        pos = 0;
                        BPUTCH('|');
                        for (i = 0; i < padl; i++) BPUTCH(' ');
                        for (li = 0; li < BIG_NLETTERS; li++) {
                            int cidx = (li + banner_frame) % BIG_NCOLORS;
                            BPUTESC(breath ? BIG_DIM[cidx] : BIG_RAINBOW[cidx]);
                            const char *seg = BIG_FONT[li][row];
                            for (ci = 0; ci < BIG_FONT_COLS; ci++) BPUTCH(seg[ci]);
                            BPUTESC(BIG_RESET);
                            if (li < BIG_NLETTERS - 1)
                                for (i = 0; i < BIG_LETTER_GAP; i++) BPUTCH(' ');
                        }
                        for (i = 0; i < padr; i++) BPUTCH(' ');
                        BPUTCH('|'); buf[pos] = '\0';
                        printf("\033[%d;1H\033[2K", 3 + row);
                ...
                for (i = 0; i < inner; i++) BPUTCH(' ');
                BPUTCH('|'); buf[pos] = '\0';
                printf("\033[8;1H\033[2K"); fwrite(buf, 1, pos, stdout);

                /* 行9：CONGRATULATIONS 变色 */
                {
                    const char *sub = "* * * * *   C O N G R A T U L A T I O N S   * * * * *";
                    int sub_dw = (int)strlen(sub);
                    int padl   = (inner - sub_dw) / 2;
                    int padr   = inner - sub_dw - padl;
                    pos = 0;
                    BPUTCH('|');
                    for (i = 0; i < padl; i++) BPUTCH(' ');
                    BPUTESC(BIG_RAINBOW[(banner_frame / 8) % BIG_NCOLORS]);
                    for (i = 0; i < sub_dw; i++) BPUTCH(sub[i]);
                    BPUTESC(BIG_RESET);
                    for (i = 0; i < padr; i++) BPUTCH(' ');
                    BPUTCH('|'); buf[pos] = '\0';
                    printf("\033[9;1H\033[2K"); fwrite(buf, 1, pos, stdout);
                }

                /* 行10（=BANNER_ROWS+1）：波浪底框 */
                pos = 0;
                BPUTESC("\033[1;97m");
                BPUTCH('+');
                for (i = 1; i < max_col - 1; i++)
                    BPUTCH(wave[(i + banner_frame + 2) % 4]);  /* +2 让上下框反相 */
                BPUTCH('+');
                BPUTESC(BIG_RESET);
                buf[pos] = '\0';
                printf("\033[%d;1H\033[2K", BANNER_ROWS);
                fwrite(buf, 1, pos, stdout);
...
#undef BPUTESC
                ...
                banner_frame++;
                if (banner_frame > 100000) banner_frame = 0;
            }
            ...
            /* 底部 HUD：偏移回真实 LINES（已加回 BANNER_ROWS）*/
            int real_lines = LINES + BANNER_ROWS;
            int hud_len = strlen(hud);
            if (hud_len > COLS - 1) hud_len = COLS - 1;
            printf("\033[%d;1H\033[K", real_lines);
