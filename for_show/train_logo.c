...
#define BANNER_ROWS 4   /* 顶部固定横幅占用的行数：顶边框+标题+副标题+底边框 */
 ...
 * --------------------------------------------------------------- */
    ...
    // while (running) {
        ...
        for (x = COLS - 1; running; --x) {
            ...
            /* --- 每帧绘制顶部固定横幅（共 BANNER_ROWS=4 行）---
             *
             *  行1: +====...====+
             *  行2: |  🎉 庆祝生态大会圆满完成 🎉  <spaces>  |
             *  行3: |  *  *  * CONGRATULATIONS *  *  *  <spaces>  |
             *  行4: +====...====+
             *
             *  宽度策略：
             *    - 用 ANSI EL（\033[K）清行后写内容，右侧 | 用绝对定位单独输出
             *    - 彻底避免多字节字符宽度猜测导致的偏移问题
             */
            {
                int w = COLS - 1;   /* 安全宽度：最右列不写，防止自动换行 */
                char border[512];
                int j;

                /* 构造边框行 +===...===+ */
                border[0] = '+';
                for (j = 1; j < w - 1; j++) border[j] = '=';
                border[w - 1] = '+';
                border[w] = '\0';

                /* 行1：顶部边框 */
                printf("\033[1;1H\033[K");
                fwrite(border, 1, w, stdout);

                /* 行4：底部边框（BANNER_ROWS 行） */
                printf("\033[%d;1H\033[K", BANNER_ROWS);
                fwrite(border, 1, w, stdout);

                /* 行2：主标题
                 *   显示宽度精确计算：
                 *   🎉 = 2, 空格 = 1, 庆祝生态大会圆满完成 = 10汉字×2 = 20, 空格 = 1, 🎉 = 2
                 *   合计显示宽度 = 2+1+20+1+2 = 26
                 *   UTF-8 字节数 = 4+1+27+1+4 = 37
                 */
                {
                    const char *title = "\xf0\x9f\x8e\x89 \xe5\xba\x86\xe7\xa5\x9d\xe7\x94\x9f\xe6\x80\x81\xe5\xa4\xa7\xe4\xbc\x9a\xe5\x9c\x86\xe6\xbb\xa1\xe5\xae\x8c\xe6\x88\x90 \xf0\x9f\x8e\x89";
                    int title_dw = 26;          /* 终端显示列宽 */
                    int inner    = w - 2;       /* | 之间可用列数 */
                    int padl     = (inner - title_dw) / 2;
                    int padr     = inner - title_dw - padl;
                    char line[512];
                    int pos = 0;
                    line[pos++] = '|';
                    memset(line + pos, ' ', padl); pos += padl;
                    memcpy(line + pos, title, strlen(title)); pos += strlen(title);
                    memset(line + pos, ' ', padr); pos += padr;
                    line[pos++] = '|';
                    line[pos] = '\0';
                ...
                /* 行3：副标题（纯 ASCII，显示宽度 = 字节数）*/
                {
                    const char *sub = "*  *  *  CONGRATULATIONS  *  *  *";
                    int sub_dw  = 33;
                    int inner   = w - 2;
                    ...
                    line[pos++] = '|';
                    memset(line + pos, ' ', padl); pos += padl;
                    memcpy(line + pos, sub, strlen(sub)); pos += strlen(sub);
                    memset(line + pos, ' ', padr); pos += padr;
                    line[pos++] = '|';
                    line[pos] = '\0';
